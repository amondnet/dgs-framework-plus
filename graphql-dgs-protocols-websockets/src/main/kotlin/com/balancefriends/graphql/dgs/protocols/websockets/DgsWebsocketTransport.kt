...
class DgsWebsocketTransport(
    ...
    private val onConnectListener: OnConnectListener? = null
) :
    ...
    public override fun handleTextMessage(session: WebSocketSession, textMessage: TextMessage) {
        ...
        when (message) {
            is Message.ConnectionInitMessage -> {
                ...
                val permittedOrPayload = onConnectListener?.invoke()
                if (permittedOrPayload?.isSuccess == false) {
                    return session.close(CloseCode.Forbidden.toCloseStatus("Forbidden"));
                }
                    ...
                    TextMessage(
                        objectMapper.writeValueAsBytes(
                            Message.ConnectionAckMessage(
                                payload = permittedOrPayload?.getOrNull()
                            )
                        )
                    )
                )
